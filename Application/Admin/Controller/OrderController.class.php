...
//        每页显示的记录数
    ...
    public function export()
    {
        $condition = array();
        $data = I("get.");
        $keys=["id值","店铺id","用户id","订单号","总价格","支付方式","支付状态","送达时间","运费","满减","备注","状态","时间","用户","详情","店铺","订单id","名称","电话","省","城市","区","地址","邮政编码"];
        ...
        foreach ($order as $key => $value) {
            ...
            if($order[$key]["payment"]==0){
                $order[$key]["payment"]="账户支付";
            }
            if($order[$key]["payment"]==1){
                $order[$key]["payment"]="微信支付";
            }
            if($order[$key]["payment"]==2){
                $order[$key]["payment"]="支付宝支付";
            }
            if($order[$key]["payment"]==3){
                $order[$key]["payment"]="货到付款";
            }
        ...
        // $item=$this->excelDataFormat($order);
