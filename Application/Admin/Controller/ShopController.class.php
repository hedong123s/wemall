...
//      每页显示的记录数
    ...
    {
        $keys=["商品id","店铺id","菜单id","商品名称","二级名称","价格","原价","单位","积分","售价","库存","图集id","访问量","psku","图片id","详情","状态","标签","备注","排名","时间","菜单名称","图片名称","图片时间","shu","shop"];
        $newOrder=array();
        
        $product = D('Product')->getList(array(), true);
        foreach ($product as $key => $value) {
            unset($product[$key]["comment"]);
            if($product[$key]["status"]==0){
                $product[$key]["status"]="售罄";
            }
            if($product[$key]["status"]==1){
                $product[$key]["status"]="上架";
            }
            if($product[$key]["status"]==-1){
                $product[$key]["status"]="下架";
            }
