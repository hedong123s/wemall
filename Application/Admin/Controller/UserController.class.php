...
class UserController extends BaseController
{
    ...
    public function exportUser(){
        $keys=["用户id","推荐人","名称","电话","email","性别","城市","省份","金钱","积分","状态" ,"是否认证","购买数","类型","等级","备注","时间"];
        $newOrder=array();
        $user = D('User')->getList(array(), false);
        foreach ($user as $key => $value) {
            unset($user[$key]["comment"]);
            unset($user[$key]["avater"]);
            unset($user[$key]["openid"]);
            unset($user[$key]["password"]);
            unset($user[$key]["token"]);
            unset($user[$key]["language"]);
            unset($user[$key]["subscribe"]);
            unset($user[$key]["lastip"]);
            unset($user[$key]["ctime"]);
            unset($user[$key]["contact_id"]);
            unset($user[$key]["nickname"]);
            unset($user[$key]["country"]);
            
            if($product[$key]["sex"]=='0'){
                $product[$key]["sex"]="";
            }
            if($product[$key]["sex"]=='1'){
                $product[$key]["sex"]="男";
            }
            if($product[$key]["sex"]=='2'){
                $product[$key]["sex"]="女";
            }
