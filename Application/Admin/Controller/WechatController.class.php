...
class WechatController extends Controller
{
    ...
    public function checkUser($openId)
    {
        ...
        if ($user) {
            ...
            $re = D("User")->add($user);
            $hooks = M("Hooks")->where(array("number" => '1001',"status" => '1'))->find();
            if($hooks){
                hook('firstFollow',array('action'=>'','id'=>$re));
            }
        }
    }
    ...
    public function getQRCode()
    {
        ...
        if ($qrcode) {
            D("Config")->add(array("id" => 1,"qrcode" => $qrcode));
            $this->success("生成二维码成功", "Admin/Config/configSet");
        } else {
            $this->error("生成二维码失败", "Admin/Config/configSet");
        }
    }
