...
class ConfigController extends BaseController
{
    ...
    public function refreshtemplateid()
    {
        ...
        $template = D("WxTplmsg")->get(array("template_id_short"=>$template_id));

        $wx = new \Wechat ($options);
        $re = $wx->addTemplateMessage($template_id);

        if($re){
            $template['template_id'] = $re;
            D("WxTplmsg")->add($template);
            $info['templateid'] = $re;
            $info['status'] = 1;
            $info['msg'] = '更新成功';
        }else{
            $info['status'] = 0;
            $info['msg'] = '更新失败,请手动添加试试！';
        }
