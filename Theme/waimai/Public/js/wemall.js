...
function login() {
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res) {
                d_messages('登录成功，跳转中', 2);
                data.username = res.username;
                data.avater = res.image;
                // location.reload();
                // window.location.href=window.location.href+"?id="+10000*Math.random();
                window.location.href=data.baseUrl + "/App/Index/index";
            } else {
                alert("登录失败!");
            }
        },
                ...
                if (res.order != undefined) {
                    ...
                    if (json.length != 0) {
                        ...
                        $.each(json, function (index, value) {
                            ...
                            htmlfirst += '<li><span style="color:#16aad8;">'+ '666' +'</span><div class="order-info"><span class="number">订单号：<span id="order-no">' + value.orderid + '</span></span><span class="date" style="float: right">' + value.time + '</span><span class="order-status">' + pay_status + ',' + order_status + '</span></div><div class="order-list" id="item-order-list"><ul>';
                            ...
                            if(value.status == -2 || value.status == 1 ){
                                post_status='<span class="commitOrder" style="width:57px" onclick= postOrder("'+ value.fahuokd +'","' + value.fahuokdnum + '")>快递查询</span>';
                                
                            }
                            htmlend += '</ul><div class="mytotal-info"><span class="deliver">运费：' + value.freight + '元</span><span class="deliver">优惠：' + value.discount + '元</span><span class="total">共' + value.totalprice + '元</span></div></div><div class="order-footer">' + cancel_status + comment_status + pay_status + post_status + '<a class="dail-small" href="tel:' + data.config.tel + '"><span class="dail-ico"><i class="ico ico-phone"></i></span><span class="dail-text">拨打电话催一催</span></a></div><div class="divider"></div></li>';
...
//订单查询
function postOrder(name,num){

    //跳出去查快递
    var url = 'http://m.kuaidi100.com/index_all.html?type='+ name +'&postid='+ num +'';
    location.href = url;
    return;
    var url=data.baseUrl + "/App/Public/kuaidi"
    $.post(url, {fahuokdnum:num},function(data){
       
       
        var html='<div><ul class="oul">';
        
        $.each(data.data,function(value,item){
           html +="<li>"+item.context+'<span style="margin-left:10px;color: #4691c1;">'+item.time+"</span>"+"</li>"
        })
        html +="</ul></div>";
        html +='<div onclick="closelay()" style="position:absolute;right:5px;top:5px;color:#b0b8bf">'+"关闭"+"</div>"
        if(data.message=="ok"){
            pageii = layer.open({
                type: 1,
                content:html,
